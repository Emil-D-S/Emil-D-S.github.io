let plot;
let harmonicSlider;
let trace; // v px
let lastPoint = null; // {x,y}

function f(x) {
  let y = Math.sign(sin(x));
  return y;
}

function isInsideCanvas(mx, my) {
  return mx >= 0 && mx < width && my >= 0 && my < height;
}

function setup() {
  createCanvas(1000, 600);
  pixelDensity(1);
  trace = new Float32Array(width).fill(NaN);

  const originY = height / 2;
  for (let px = 0; px < width; px++) {
    // map pixel x to function domain [0, TWO_PI]
    const x = map(px, 0, width, 0, TWO_PI);
    const y = f(x);
    // map function y to pixel space (inverted because canvas y grows downward)
    const py = originY - y * 200; // 200 is scaleY from FunctionGrapher
    trace[px] = py;
  }

  harmonicSlider = createSlider(0, 100, 20);
  harmonicSlider.position(15, height);

  let origoFuncTrace = new Trace(f, {
    strokeWeight: 5,
    strokeColor: "red",
  });

  let dftFuncTrace = new Trace(
    () => {
      return 0;
    },
    {
      strokeWeight: 5,
      strokeColor: "yellow",
    }
  );

  plot = new FunctionGrapher([origoFuncTrace, dftFuncTrace], {
    startX: 0,
    endX: TWO_PI,
    scaleY: 200,
    showAxes: true,
    axesCol: "gray",
    strokeWeight: 1,
    showMarks: false,
    strokeCol: "red",
  });

  let z = dftFromPoints(plot.scaleTraceToFunction(trace));

  plot.traces[1].func = (x) => {
    let F = z.A0;
    const K = Math.min(harmonicSlider.value(), z.amps.length - 1);
    for (let k = 1; k <= K; k++) {
      F += z.amps[k] * Math.cos(k * x + z.phases[k]);
    }
    return F;
  };
}

function draw() {
  background(20);
  plot.draw();

  fill(255);
  noStroke();

  textAlign(LEFT);
  textSize(30);
  text("Draw a function", 20, 40);

  textAlign(LEFT);
  textSize(15);
  text("number of harmonics: " + harmonicSlider.value(), 15, height - 10);
}

function mousePressed() {
  if (!isInsideCanvas(mouseX, mouseY)) return;

  lastPoint = {
    x: constrain(mouseX, 0, width - 1),
    y: constrain(mouseY, 0, height - 1),
  };
  trace[Math.round(lastPoint.x)] = lastPoint.y;
}

function mouseDragged() {
  if (!isInsideCanvas(mouseX, mouseY)) return;
  const x2 = constrain(mouseX, 0, width - 1);
  const y2 = constrain(mouseY, 0, height - 1);
  if (!lastPoint) {
    lastPoint = { x: x2, y: y2 };
  }

  const x1 = Math.round(lastPoint.x);
  const y1 = lastPoint.y;
  const xEnd = Math.round(x2);
  const dx = xEnd - x1;

  if (dx === 0) {
    trace[xEnd] = y2;
  } else {
    const step = dx > 0 ? 1 : -1;
    for (let xi = x1; xi !== xEnd + step; xi += step) {
      const t = (xi - x1) / dx; // 0..1
      const yi = y1 + t * (y2 - y1); // linpol
      if (xi >= 0 && xi < width) trace[xi] = yi;
    }
  }

  lastPoint = { x: x2, y: y2 };
}

function mouseReleased() {
  lastPoint = null;

  if (typeof plot.setFunction === "function") {
    plot.setFunction(trace);
  }

  let z = dftFromPoints(plot.scaleTraceToFunction(trace));

  plot.traces[0].setFunction(trace);

  plot.traces[1].func = (x) => {
    let F = z.A0;
    const K = Math.min(harmonicSlider.value(), z.amps.length - 1);
    for (let k = 1; k <= K; k++) {
      F += z.amps[k] * Math.cos(k * x + z.phases[k]);
    }
    return F;
  };
}
